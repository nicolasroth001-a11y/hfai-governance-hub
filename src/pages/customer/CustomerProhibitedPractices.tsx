...
const PROHIBITED_PRACTICES = [
  {
    ...
    article: "Art. 5(1)(a)",
    description: "AI that deploys subliminal techniques beyond a person's consciousness to materially distort behaviour causing significant harm.",
    ruleKeywords: ["manipulation", "subliminal", "coercion"],
    icon: Brain,
    severity: "critical",
  },
  {
    ...
    article: "Art. 5(1)(a)",
    description: "AI exploiting vulnerabilities due to age, disability, or social/economic situation to distort behaviour causing significant harm.",
    ruleKeywords: ["exploit", "vulnerable", "disability", "elderly"],
    icon: Users,
    severity: "critical",
  },
  {
    ...
    article: "Art. 5(1)(c)",
    description: "AI-based evaluation or classification of persons based on social behaviour leading to detrimental or unfavourable treatment.",
    ruleKeywords: ["social score", "social scoring", "social credit", "citizen score"],
    icon: Ban,
    severity: "critical",
  },
  {
    ...
    article: "Art. 5(1)(d)",
    description: "AI predicting criminal offence risk based solely on profiling or personality traits assessment.",
    ruleKeywords: ["predictive policing", "crime prediction", "criminal profiling"],
    icon: Eye,
    severity: "critical",
  },
  {
    ...
    article: "Art. 5(1)(e)",
    description: "Creating or expanding facial recognition databases through untargeted scraping from internet or CCTV footage.",
    ruleKeywords: ["facial scraping", "face database", "biometric scraping"],
    icon: Fingerprint,
    severity: "critical",
  },
  {
    ...
    article: "Art. 5(1)(f)",
    description: "Inferring emotions in workplace or educational institutions, except for medical or safety reasons.",
    ruleKeywords: ["emotion recognition", "emotion detection", "sentiment workplace"],
    icon: Eye,
    severity: "high",
  },
  {
    id: "biometric_categorisation",
    title: "Biometric Categorisation (Sensitive Data)",
    article: "Art. 5(1)(g)",
    description: "Categorising individuals based on biometric data to deduce race, political opinions, religious beliefs, sexual orientation.",
    ruleKeywords: ["biometric categorisation", "biometric classification", "race detection"],
    icon: Fingerprint,
    severity: "critical",
  },
  {
    ...
    ruleKeywords: ["real-time biometric", "remote identification", "facial recognition public"],
    icon: Eye,
    severity: "critical",
  },
];
...
export default function CustomerProhibitedPractices() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      supabase.from("ai_systems").select("id, name, eu_risk_tier, description").eq("org_id", profile.org_id),
    ]).then(([rulesRes, systemsRes]) => {
  ...
  const getPracticeStatus = (practice: typeof PROHIBITED_PRACTICES[0]) => {
    ...
    return hasRule ? "monitored" : "unmonitored";
  };

  const monitoredCount = PROHIBITED_PRACTICES.filter(p => getPracticeStatus(p) === "monitored").length;
  const unmonitoredCount = PROHIBITED_PRACTICES.length - monitoredCount;
              ...
              <Card key={practice.id} className={status === "unmonitored" ? "border-destructive/30" : ""}>
                <CardContent className="p-5">
                  <div className="flex items-start gap-4">
                    <div className={`h-10 w-10 rounded-xl flex items-center justify-center shrink-0 ${status === "monitored" ? "bg-primary/10" : "bg-destructive/10"}`}>
                      <Icon className={`h-5 w-5 ${status === "monitored" ? "text-primary" : "text-destructive"}`} />
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 flex-wrap mb-1">
                        ...
                        <Badge
                          variant={status === "monitored" ? "default" : "destructive"}
                          className="text-[10px] ml-auto"
                        >
                          {status === "monitored" ? "Rule Active" : "No Rule"}
                        </Badge>
                      </div>
