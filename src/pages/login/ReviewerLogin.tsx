...
export default function ReviewerLogin() {
  ...
  if (showMFA) {
    return (
      <MFAChallenge
        onVerified={() => navigate("/reviewer/dashboard")}
        onCancel={() => { setShowMFA(false); logout(); }}
      />
    );
  }
    ...
    <div className="min-h-screen bg-background flex items-center justify-center p-6">
      ...
      <Card className="w-full">
        <CardHeader className="text-center space-y-3">
          ...
          <CardTitle className="text-xl">{t("auth.reviewerPortal")}</CardTitle>
          <CardDescription>{t("auth.reviewerPortalDesc")}</CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          <form onSubmit={handleLogin} className="space-y-4">
            <div className="space-y-2">
              <Label htmlFor="email" className="text-card-foreground">{t("auth.email")}</Label>
              <Input id="email" type="email" placeholder="reviewer@hfai.com" value={email} onChange={(e) => setEmail(e.target.value)} required className="bg-card border-card-foreground/10" />
            </div>
            <div className="space-y-2">
